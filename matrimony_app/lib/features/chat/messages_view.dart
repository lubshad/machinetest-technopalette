...
import 'package:matrimony_app/features/chat/user_provider.dart';
...
class _MessagesViewWrappedState extends State<MessagesViewWrapped>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: UserProviderWidget(
        child: MessagesView(
          onItemTap: (context, model, rect) {
            if (model.message.body.type != MessageType.FILE) return false;
            final fileBody = model.message.body as ChatFileMessageBody;
            if (fileBody.fileStatus != DownloadStatus.SUCCESS) {
              ChatClient.getInstance.chatManager.downloadAttachment(
                model.message,
              );
              return false;
            }
            // OpenFile.open(fileBody.localPath);
            return true;
          },
          enableAppBar: false,
